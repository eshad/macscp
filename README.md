...
MacSCP uses the built-in macOS SSH tools — no libraries or frameworks needed:

| Operation | Command |
|-----------|---------|
| List files | `ssh user@host "ls -la /path"` |
| Download | `scp -r user@host:/remote /local` |
| Upload | `scp -r /local user@host:/remote` |
| Delete | `ssh user@host "rm -rf /path"` |
| Rename | `ssh user@host "mv /old /new"` |
| Create folder | `ssh user@host "mkdir -p /path"` |

Password authentication uses `SSH_ASKPASS` with a temporary helper script. SSH key auth works through ssh-agent or a specified key file.
